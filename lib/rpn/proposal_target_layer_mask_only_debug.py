...
class ProposalTargetLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        # targets
        labels, rois, bbox_targets, bbox_inside_weights,mask_targets,rois_gt = _sample_rois(
            all_rois, gt_boxes, gt_masks,fg_rois_per_image,
            rois_per_image, self._num_classes)


        print len(labels),'len lablels'
        for i in np.arange(len(labels)):
            if int(labels[i]) != 0:
                ...
                cv2.rectangle(image,(int(rois_gt[i][0]),int(rois_gt[i][1])),(int(rois_gt[i][2]),int(rois_gt[i][3])),(0,0,255),1)
                cv2.putText(image,str(j),(int(rois_gt[i][0]),int(rois_gt[i][1])),cv2.FONT_HERSHEY_COMPLEX,1,(0,0,255))
                #cv2.putText()
...
def _sample_rois(all_rois, gt_boxes,gt_masks,fg_rois_per_image, rois_per_image, num_classes):
    ...
    return labels, rois, bbox_targets, bbox_inside_weights, mask_targets,gt_boxes[gt_assignment[keep_inds],:4]
