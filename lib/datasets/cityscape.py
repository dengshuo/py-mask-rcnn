...
class Cityscape(imdb):
    def __init__(self, image_set, root_path, dataset_path):
        ...
        :return: imdb object
        """
        super(Cityscape, self).__init__('cityscape')
    ...
    def load_from_seg(self, ins_seg_path):
        seg_gt = os.path.join(self.data_path,'gtFine_trainvaltest', ins_seg_path)
        # print seg_gt
        ...
        for c in range(1, len(self.class_id)):
            ...
            for id in ids:
                ...
                mask_encoded = maskUtils.encode(fortran_mask)
                mask_encoded['counts'] = mask_encoded['counts'].decode('ascii')
                gt_masks.append(json.dumps(mask_encoded))
        return np.asarray(boxes), np.asarray(gt_classes), np.asarray(ins_id), seg_gt, np.asarray(gt_overlaps),gt_masks
    ...
    def load_cityscape_annotations(self):
        ...
        assert len(imgfiles_list) == self._num_images, 'number of boxes matrix must match number of images'
        roidb = []
        for im in range(self._num_images):
            # print '===============================', im, '====================================='
            roi_rec = dict()
            roi_rec['image'] = os.path.join(self.data_path,'leftImg8bit_trainvaltest', imgfiles_list[im]['img_path'])
            size = cv2.imread(roi_rec['image']).shape
            roi_rec['height'] = size[0]
            roi_rec['width'] = size[1]
            boxes, gt_classes, ins_id, pixel, gt_overlaps,gt_masks = self.load_from_seg(imgfiles_list[im]['ins_seg_path'])
            if boxes.size == 0:
                total_num_objs = 0
            ...
            roi_rec['masks'] = gt_masks
            assert len(roi_rec) == 12
    ...
    def append_flipped_images(self):
        ...
        assert self._num_images == len(self.roidb)
        for i in range(self._num_images):
            ...
            entry = {'image': roi_rec['image'],
                     ...
                     'ins_seg': self.roidb[i]['ins_seg'],
                     'ins_id': self.roidb[i]['ins_id'],
