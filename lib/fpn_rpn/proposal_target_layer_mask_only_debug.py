...
class ProposalTargetLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        labels, rois, bbox_targets, bbox_inside_weights,mask_targets = _sample_rois(
            ...
            rois_per_image, self._num_classes,image)
...
def _sample_rois(all_rois, gt_boxes,gt_masks,fg_rois_per_image, rois_per_image, num_classes):
    ...
    for i in np.arange(len(labels)):
        if int(labels[i]) == 0:
            ...
            image_ori[0,np.round(y1).astype(np.int):mask_roi.shape[0]+np.round(y1).astype(np.int),np.round(x1).astype(np.int):mask_roi.shape[1]+np.round(x1).astype(np.int)][mask_roi<0.5]=(0,0,0)
