...
class ProposalTargetLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        # mask_targets
        top[5].reshape(1,self._num_classes-1,cfg.MASK_SIZE,cfg.MASK_SIZE)

        #keypoints mask_targets
        top[6].reshape(1,17)
    ...
    def forward(self, bottom, top):
        ...
        # targets
        labels, rois, bbox_targets, bbox_inside_weights,mask_targets,kps_labels = _sample_rois(
        ...
        #kps_targets
        top[6].reshape(*kps_labels.shape)
        top[6].data[...] = kps_labels
...
def _sample_rois(all_rois, gt_boxes,gt_masks,gt_keypoints,fg_rois_per_image, rois_per_image, num_classes):
    ...
    mask_targets[:,:,:,:] = -1
    kps_labels = np.zeros((len(labels),cfg.KEYPOINTS_SIZE),dtype=np.float32)
    kps_labels[:,:]=-1
    ...
    #             mask_targets[i, 0, :, :] = -1
    #         if(j == int(labels[i])):
    #             mask_targets[i, j, :, :] = gt_masks[gt_assignment[i]]
    #         else:
    #             mask_targets[i, j, :, :] = -1
    ...
    for i in np.arange(len(labels)):
        if int(labels[i]) == 0:
            ...
            #keypoinsts;personal
            if(int(labels[i]) == 1):
                gt_person_keypoints = gt_keypoints[keep_assingment_inds[i],:]
                for index in xrange(cfg.KEYPOINTS_SIZE):
                    x = gt_person_keypoints[3*index]
                    y = gt_person_keypoints[3*index+1]
                    v = gt_person_keypoints[3*index+2]
                    # kps_mask = np.zeros((1,cfg.KEYPOINTS_MASK_SIZE,KEYPOINTS_MASK_SIZE))
                    if v >0.01: 
                        if x > x1 and x < x2 and y > y1 and y < y2:
                            x_ratio = (x-x1)/(x2-x1)
                            y_ratio = (y-y1)/(y2-y1)
                            x_resized = np.round(cfg.KEYPOINTS_MASK_SIZE*x_ratio).astype(np.int)
                            y_resized = np.round(cfg.KEYPOINTS_MASK_SIZE*y_ratio).astype(np.int)
                            if x_resized == cfg.KEYPOINTS_MASK_SIZE:
                                x_resized -=1
                            if y_resized == cfg.KEYPOINTS_MASK_SIZE:
                                y_resized -=1
                            # kps_mask[x_resized,y_resized]=1
                            kps_labels[i,index] = y_resized*cfg.KEYPOINTS_MASK_SIZE+x_resized
                # print kps_labels[i]
        else:
            mask_targets[i, j-1, :, :] = -1
            kps_labels[i,:] = -1
    ...
    return labels, rois, bbox_targets, bbox_inside_weights, mask_targets, kps_labels
