...
def get_minibatch(roidb, num_classes):
    ...
    padded_im_blob,padded_image,im_blob,im_scales = _get_image_blob(roidb, random_scale_inds)
    ...
    if cfg.TRAIN.HAS_RPN:
        ...
        blobs['im_info'] = np.array(
            [[padded_im_blob.shape[2], padded_im_blob.shape[3], im_scales[0]]],
            dtype=np.float32)
        blobs['gt_masks'] = gt_masks
        blobs['image'] = padded_image
...
def _get_image_blob(roidb, scale_inds):
    ...
    im_ori = cv2.imread(roidb[0]['image'])
    if roidb[i]['flipped']:
        im_ori = im_ori[:, ::-1, :]
    target_size = cfg.TRAIN.SCALES[scale_inds[i]]
    padded_image,image,im_scale = prep_im_original_for_blob(im_ori, cfg.PIXEL_MEANS, target_size,
                                    cfg.TRAIN.MAX_SIZE,cfg.TRAIN.IMAGE_STRIDE)
    # original_ims = []
    # for i in xrange(num_images):
    ...
    return padded_bolb,padded_image,blob,im_scales
