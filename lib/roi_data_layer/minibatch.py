...
def get_minibatch(roidb, num_classes):
    ...
    if cfg.TRAIN.HAS_RPN:
        ...
        gt_keypoints[:] = roidb[0]['keypoints'][gt_inds,:] * im_scales[0]
        masks_encoded=[]
        for i in gt_inds:
        ...
        if roidb[0]['flipped'] is True:
            width = roidb[0]['width'] * im_scales[0]
            kps = gt_keypoints.copy()
            oldx1 = kps[:, 0::3].copy()
            # boxes[:, 0] = widths[i] - oldx2 - 1
            # boxes[:, 2] = widths[i] - oldx1 - 1
            kps[:, 0::3] = width - oldx1 - 1
            kps_index_map=[0,2,1,4,3,6,5,8,7,10,9,12,11,14,13,16,15]
            kps_flipped = np.empty((gt_keypoints.shape[0],gt_keypoints.shape[1]),dtype=np.float32)
            for index in xrange(len(kps_index_map)):
                kps_flipped[:,index*3]=kps[:,kps_index_map[index]*3]
                kps_flipped[:,index*3+1]=kps[:,kps_index_map[index]*3+1]
                kps_flipped[:,index*3+2]=kps[:,kps_index_map[index]*3+2]
            gt_keypoints = kps_flipped
        blobs['gt_keypoints'] = gt_keypoints
