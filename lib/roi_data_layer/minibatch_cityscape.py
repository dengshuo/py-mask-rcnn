...
def get_minibatch(roidb, num_classes):
    ...
    if cfg.TRAIN.HAS_RPN:
        ...
        gt_ins = np.empty((len(gt_inds)),dtype=np.int)
        gt_boxes[:, 0:4] = roidb[0]['boxes'][gt_inds, :] * im_scales[0]
        gt_boxes[:, 4] = roidb[0]['gt_classes'][gt_inds]
        gt_ins[:]=roidb[0]['ins_id'][gt_inds]
        gt_class_id = [0, 24, 25, 26, 27, 28, 31, 32, 33]
        masks_encoded=[]
        for i in gt_inds:
        ...
        #     #mask = annToMask(masks_encoded[i],roidb[0]['height'],roidb[0]['width'])
        #     # for tuzhen dataset
        #     mask = np.zeros((im.size[1], im.size[0]), dtype=np.float32)
        #     c = roidb[0]['gt_classes'][gt_inds[i]]
        #     px = np.where((pixel >= gt_class_id[c] * 1000) & (pixel < (gt_class_id[c] + 1) * 1000))
        #     if len(px[0]) == 0:
        #         continue
        ...
        #     if roidb[0]['flipped'] is True:
        #         mask = np.fliplr(mask)
        #     mask_ = cv2.resize(mask, (im_blob.shape[3], im_blob.shape[2]),interpolation=cv2.INTER_CUBIC)
        #     gt_masks[i,:,:] = mask_
