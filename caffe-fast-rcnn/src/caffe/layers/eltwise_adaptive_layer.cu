...
template <typename Dtype>
void EltwiseAdaptiveLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  case EltwiseParameter_EltwiseOp_SUM:
    ...
    caffe_gpu_axpy(count, coeffs_[0], crop_top_blob_.gpu_data(), top_data);
    //bottom[0] = &crop_top_blob_;
...
template <typename Dtype>
void EltwiseAdaptiveLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    ...
    if (propagate_down[i]) {
      ...
      case EltwiseParameter_EltwiseOp_SUM:
        if (coeffs_[i] == Dtype(1.)) {
          ...
          const Dtype* top_diff = crop_bottom_vec_[0]->gpu_diff();
...
INSTANTIATE_LAYER_GPU_FUNCS(EltwiseAdaptiveLayer);
