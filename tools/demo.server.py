...
def vis_detections(im, class_name,cls_ind, dets, masks,thresh=0.5):
    ...
    for i in inds:
        ...
        im_clone = im.copy()
        im_clone[int(bbox[1]):int(bbox[3]),int(bbox[0]):int(bbox[2])][mask_restore<0] =(0,0,0)
        cv2.rectangle(im_clone,(int(bbox[0]),int(bbox[1])),(int(bbox[2]),int(bbox[3])),(0,0,255),1)
        file_name = string.join(random.sample(['a','b','c','d','e','f','g','h','i','j','k','l','m','n'], 6)).replace(" ","")
        cv2.imwrite(file_name+'.jpg',im_clone)
...
if __name__ == '__main__':
    ...
    # im_names = ['COCO_test2014_000000000014.jpg']
